...
class UcodeAPIClient {
  ...
  async invokeFunction({ method, data = {}, authToken: providedToken = null, headers: additionalHeaders = {} }) {
    ...
    // Для auth методов передаем environment-id в заголовках, для остальных - в query параметрах
    const url = isAuthMethod
      ? `${this.baseURL}${this.invokeFunctionEndpoint}?project-id=${this.projectId}`
      : `${this.baseURL}${this.invokeFunctionEndpoint}?project-id=${this.projectId}&environment-id=${this.environmentId}`
    ...
    console.log(`U-code API [${method}] request:`, {
      ...
      headers: { 
        ...
        'environment-id': headers['environment-id'] || 'in query params'
      },
      bodyObject: body,
