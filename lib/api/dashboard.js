...
export const dashboardAPI = {
  ...
  getChartOfAccountsV2: async (params = {}) => {
    // Use new API through chartOfAccountsAPI
  ...
  deleteMyAccounts: async (ids) => {
    // Import bankAccountsAPI to use the new direct API method
    const { bankAccountsAPI } = await import('./ucode/bankAccounts')
    
    // Delete each account using the new API method
    const deletePromises = ids.map(guid => bankAccountsAPI.deleteMyAccount(guid))
